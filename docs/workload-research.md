...
При коротком прогоне (`-duration 30s`) 1–2 ошибки в конце — скорее **shutdown race**: parent context отменяется, in-flight запросы получают deadline exceeded. Не обязательно медленные запросы (p95 ~700ms).

**Mitigation:** graceful shutdown (дождать in-flight ops), или игнорировать на smoke; для чистого отчёта — duration +1–2s.
